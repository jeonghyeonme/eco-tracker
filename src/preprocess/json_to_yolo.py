    ...
    try:
        with zipfile.ZipFile(img_zip_path, 'r') as img_zip, zipfile.ZipFile(lbl_zip_path, 'r') as lbl_zip:
            ...
            for json_name in json_names:
                ...
                if rand_val < split_ratios[0]: split = 'train'
                elif rand_val < split_ratios[0] + split_ratios[1]: split = 'val'
                else: split = 'test'
                ...
                try: img_w, img_h = map(float, res_str.split('/'))
                except: img_w, img_h = 1920.0, 1080.0
                ...
                for obj in data.get('objects', []):
                    ...
                    yolo_labels.append(f"{CLASS_MAP[class_code]} {(x+w/2)/img_w:.6f} {(y+h/2)/img_h:.6f} {w/img_w:.6f} {h/img_h:.6f}")
                
                if not yolo_labels: continue
                
                # 2. Image
                img_name = Path(json_name).stem + '.jpg'
                ...
                cv2.imwrite(str(output_root / split / 'images' / f"{stem}.jpg"), img_resized)
                with open(output_root / split / 'labels' / f"{stem}.txt", 'w', encoding='utf-8') as f:
                    f.write('\n'.join(yolo_labels))
    except Exception as e:
        print(f"Error processing {img_zip_path}: {e}")
...
def main():
    ...
    img_zips = sorted(list(Path(args.img_zip_dir).rglob('*.zip')))
    lbl_zips = sorted(list(Path(args.lbl_zip_dir).rglob('*.zip')))
    
    zip_pairs = []
    for iz in img_zips:
        # Match based on the suffix like B1, B2, B3...
        suffix = iz.stem.split('_')[-1]
        lz = next((l for l in lbl_zips if l.stem.endswith(suffix)), None)
        if lz: zip_pairs.append((iz, lz))
        
    print(f"Found {len(zip_pairs)} pairs of ZIP files. Starting processing...")
    
    worker = partial(process_zip_pair, output_root=output_root, split_ratios=args.split)
    with Pool(cpu_count()) as p:
        list(tqdm(p.imap(worker, zip_pairs), total=len(zip_pairs), desc="Processing"))
